...
void loop()
{
  ...
  {
    OTA_Restart();
  } // checks if the esp needs a reboot        - done via webpage / OTA - customserver.h
  if (factoryRequired)
  {
    deleteData();
  } // checks if the esp needs a factory reset - done via webpage / OTA - customserver.h and wifisave.h
